<template>
  <div class="container">
    ...
    <div
      ...
      v-for="history of transformedHistory"
      class="history-container"
    >
      <div>
        <p class="date" :class="{ current: format(new Date(history[0].date)) === currentDate }">
          {{ format(new Date(history[0].date)) }}
        </p>
      </div>
...
onMounted(async () => {
  ...
  date.value = [startDate, currentDate]
  await getHistory()
})
